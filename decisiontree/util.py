...
def entropy(class_y):
    ...
    for i in range(len(prob)):
        if prob[i] != 0:
            entropy[i] = -prob[i] * np.math.log(prob[i], 2)
        else:
            entropy[i] = 0
...
def information_gain(previous_y, current_y):
    ...
    counts = [len(c) for c in current_y]

    assert(sum(counts) == len(previous_y))

    class_prob = [0]*len(current_y)
    class_entropy = [0]*len(current_y)
    remainder = 0.0

    for i in range(len(current_y)):
        class_prob[i] = len(current_y[i])/float(len(previous_y))
        class_entropy[i] = entropy(current_y[i])
