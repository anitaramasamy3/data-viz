...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        indices = np.random.choice(len(XX), n, replace=True)

        samples = [XX[i][:-1] for i in indices] # sampled dataset
        labels = [XX[i][-1] for i in indices]  # class labels for the sampled records
    ...
    def fitting(self):
        ...
        for i in range(self.num_trees):
            tree = DecisionTree()
            # print self.bootstraps_datasets[i]
            # print self.bootstraps_labels[i]
            self.decision_trees.append(tree)
            self.decision_trees[i].tree = tree.learn(self.bootstraps_datasets[i], self.bootstraps_labels[i])
            # print self.decision_trees[i].tree
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                # pass
                # print 'Voted randomly'
                y = np.append(y, np.random.randint(low=0,high=2,size=1))
            else:
...
def main():
    ...
    # with open("hw4-data_test.csv") as f:
        next(f, None)
